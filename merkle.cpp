...
#include <vector>
#include <string>
#include <type_traits> // Include for std::is_same_v
#include "merkle.hpp"

// Implementation of Node<T> constructors and member functions
template <typename T>
Node<T>::Node(const T &data) {
    hash = hashFunction(data);
    left = nullptr;
...
// Implementation of MerkleTree<T> constructor and member functions
template <typename T>
MerkleTree<T>::MerkleTree(std::vector<T> &data) {
    root = buildTree(data, 0, data.size() - 1);
    originalRootHash = root->hash;
...
// Verifies integrity by comparing current hash value to old hash value
template <typename T>
bool MerkleTree<T>::verifyDataIntegrity() {
    return root->hash == originalRootHash;
}

// Returns the hash value of the root node
template <typename T>
int MerkleTree<T>::getRootHash() {
    return root->hash;
}
